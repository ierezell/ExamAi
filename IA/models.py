...
from settings import MAX_LENGTH, DEVICE, SOS_token
...
class GreedySearchDecoder(nn.Module):
    ...
    def forward(self, input_seq, input_length, max_length):
        ...
        # Prepare encoder's final hidden layer to be first hidden input
        # to the decoder
        decoder_hidden = encoder_hidden[:self.decoder.n_layers]
        # Initialize decoder input with SOS_token
        decoder_input = torch.ones(1, 1,
                                   device=DEVICE, dtype=torch.long) * SOS_token
...
def chat(encoder, decoder, searcher, voc):
    input_sentence = ''
    while(1):
        try:
            # Get input sentence
