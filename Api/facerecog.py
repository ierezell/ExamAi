...
predictor_68_point_model = dat_models.pose_predictor_model_location()
pose_predictor_68_point = dlib.shape_predictor(predictor_68_point_model)

predictor_5_point_model = dat_models.pose_predictor_five_point_model_location()
pose_predictor_5_point = dlib.shape_predictor(predictor_5_point_model)

cnn_face_detection_model = dat_models.cnn_face_detector_model_location()
cnn_face_detector = dlib.cnn_face_detection_model_v1(cnn_face_detection_model)

face_recognition_model = dat_models.face_recognition_model_location()
...
def _rect_to_css(rect):
    ...
    :return: a plain tuple representation of the rect in
...
def _trim_css_to_bounds(css, image_shape):
    ...
    :return: a trimmed plain tuple representation of the rect in
                (top, right, bottom, left) order
    """
...
def load_image_file(file, mode='RGB'):
    ...
    :param mode: format to convert the image to.
                    Only 'RGB' (8-bit RGB, 3 channels) and
...
def _raw_face_locations(img, number_of_times_to_upsample=1, model="hog"):
    ...
    :param model: Which face detection model to use. "hog" is less accurate
                    but faster on CPUs. "cnn" is a more accurate
                  deep-learning model which is GPU/CUDA accelerated
                  (if available). The default is "hog".
...
def face_locations(img, number_of_times_to_upsample=1, model="hog"):
    ...
    :param number_of_times_to_upsample: How many times to upsample the image
                                        ...
                                        Higher numbers find smaller faces.
    :param model: Which face detection model to use. "hog" is less accurate
                    but faster on CPUs. "cnn" is a more accurate deep-learning
                    model which is GPU/CUDA accelerated (if available).
                    The default is "hog".
    :return: A list of tuples of found face locations in css
            (top, right, bottom, left) order
    """
    if model == "cnn":
        return [_trim_css_to_bounds(_rect_to_css(face.rect), img.shape)
                for face in _raw_face_locations(img,
...
def batch_face_locations(images, number_of_times_to_upsample=1,
    ...
    def convert_cnn_detections_to_css(detections):
        return [_trim_css_to_bounds(_rect_to_css(face.rect), images[0].shape)
...
def face_encodings(face_image, known_face_locations=None, num_jitters=1):
    """
    Given an image, return the 128-dimension face encoding for each face in
    the image.
    ...
    :param num_jitters: How many times to re-sample the face when calculating
                        encoding. Higher is more accurate,
...
def compare_faces(known_face_encodings, face_encoding_to_check, tolerance=0.6):
    ...
    """
    return list(face_distance(known_face_encodings,
